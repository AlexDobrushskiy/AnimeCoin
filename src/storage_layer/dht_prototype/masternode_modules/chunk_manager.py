...
class ChunkManager:
    ...
    async def async_process(self, ident, msg):
        logging.debug("Received RPC message: %s, ident: %s" % (msg, ident))
        reply = b'PONG'
        await self.__zmq.send_multipart([ident, reply])
        logging.debug("Sent RPC response: %s" % reply)

    async def zmq_run_forever(self):
        while True:
            ident, msg = await self.__zmq.recv_multipart()  # waits for msg to be ready
            asyncio.ensure_future(self.async_process(ident, msg))
