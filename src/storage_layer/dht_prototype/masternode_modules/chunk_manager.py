...
class ChunkManager:
    ...
    def load_full_chunks(self, chunks):
        ...
        for chunk_str, data in chunks:
            ...
            ours = self.__calculate_chunk_ownership(chunkid)
            if ours:
    ...
    def __rpc_spotcheck(self, data):
        # NOTE: data is untrusted!
        if set(data.keys()) != set(["chunkid", "start", "end"]):
        ...
        return serialize("SPOTCHECK_RESP", {"digest": digest})

    async def __zmq_process(self, ident, msg):
        self.__logger.debug("Received RPC message: %s, ident: %s" % (msg, ident))
        rpcname, data = deserialize(msg)
        self.__logger.debug("Decoded RPC packet to: %s, data: %s" % (rpcname, data))

        # TODO: validate every field
        if rpcname == "SPOTCHECK_REQ":
            reply_packet = self.__rpc_spotcheck(data)
        else:
            # TODO: signal error
            pass

        await self.__zmq.send_multipart([ident, reply_packet])
        self.__logger.debug("Sent RPC response: %s" % reply_packet)
    ...
    async def issue_random_tests_forever(self, waittime):
        while True:
            ...
            for chunkid in chunks:
                self.__logger.debug("Selected chunk %s for random check" % chunkid)
                ...
                for owner in owners:
                    ...
                    self.__logger.debug("SPOTCHECK REQUEST to %s, chunkid: %s" % (owner, chunkid))
                    response_digest = await mn.spotcheck(chunkid, start, end)
                    self.__logger.debug("SPOTCHECK RESPONSE from %s, msg: %s" % (owner, response_digest))
