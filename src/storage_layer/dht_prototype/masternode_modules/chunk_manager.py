...
class ChunkManager:
    def __init__(self, nodeid, masternode_settings, chunks, masternode_list):
        ### START - variables
        # our node id
    ...
    def __init_chunk_table(self, chunks):
        ...
        logging.debug("Added %s chunks to chunk table" % len(self.__chunk_table))
    ...
    def __init_alias_digests(self):
        for i in range(self.__masternode_settings.REPLICATION_FACTOR):
            ...
            logging.debug("Alias digest %s -> %s" % (i, digest_int))
    ...
    def __discover_files_in_local_storage(self):
        ...
        for chunkid in self.__storage.index():
            ...
            if not self.__storage.verify(chunkid):
                logging.warning("Verify failed for chunkid %s, deleting" % chunkid)
        ...
        logging.debug("Discovered %s files in local storage" % len(self.__file_db))
    ...
    def __calculate_chunk_ownership(self, chunkid):
        ...
        if owned:
            ...
            if not chunk.is_ours:
                # logging.debug("Chunk %s is now OWNED" % chunkid)
                chunk.is_ours = True
            ...
            if chunk is not None:
                if chunk.is_ours:
                    # logging.debug("Chunk %s is now DISOWNED" % chunkid)
                    chunk.is_ours = False
    ...
    def __purge_orphaned_files(self):
        logging.info("Purging orphaned files")
    ...
    def __purge_orphaned_db_entries(self):
        logging.info("Purging orphaned DB entries")
        to_delete = []
        for chunkid, chunk in self.__file_db.items():
    ...
    def update_mn_list(self, masternode_list):
        ...
        if len(added) + len(removed) > 0:
            logging.info("MN list has changed -> added: %s, removed: %s" % (added, removed))
            self.__recalculate_ownership_of_all_chunks()
    ...
    # DEBUG FUNCTIONS
    def __dump_internal_stats(self):
        logging.info("Aliases: %s, file_db: %s, chunk_table: %s" % (len(self.__alias_db),
                                                                    len(self.__file_db),
