...
def generate_scriptpubkeys(data, balance):
    txouts = []
    while len(data) > 0:
        scriptPubKey = checkmultisig_scriptPubKey_dump(data[:65*3])
        data = data[65*3:]
        ...
        if balance < 0:
            raise RuntimeError("Insufficient coins")
    return txouts, balance


def checkmultisig_scriptPubKey_dump(data):
    ret = pushint(1)
    n = 0
    while data:
        chunk = data[0:65]
        ...
        ret += pushdata(chunk)
        n += 1
    ret += pushint(n) + OP_CHECKMULTISIG
    return ret
