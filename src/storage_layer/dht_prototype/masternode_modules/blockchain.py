...
class BlockChain:
    ...
    def store_data_in_utxo(self, input_data):
        ...
        unspent = list(self.jsonrpc.listunspent())
        ...
        change -= out_value
        receiving_animecoin_blockchain_address = self.jsonrpc.getnewaddress()
        ...
        change_address = self.jsonrpc.getnewaddress()  # change output
        ...
        signed_tx = self.jsonrpc.signrawtransaction(hexlify(tx).decode('utf-8'))
        fee = Decimal(len(signed_tx['hex']) / 1000) * FEEPERKB
        change -= fee
        txouts[-1][0] = change
        final_tx = packtx(txins, txouts)
        signed_tx = self.jsonrpc.signrawtransaction(hexlify(final_tx).decode('utf-8'))
        ...
        send_raw_transaction_result = self.jsonrpc.sendrawtransaction(hex_signed_transaction)
    ...
    def retrieve_data_from_utxo(self, blockchain_transaction_id):
        raw = self.jsonrpc.getrawtransaction(blockchain_transaction_id)
