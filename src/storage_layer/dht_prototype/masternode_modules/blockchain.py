...
class BlockChain:
    ...
    def store_data_in_utxo(self, input_data):
        # print('Now storing preparing file for storage in blockchain. Original uncompressed file size in bytes: %s' % len(input_data))

        uncompressed_data_file_hash = get_digest(input_data)
        # print("HASH", hexlify(uncompressed_data_file_hash))
        (txins, change) = select_txins(0, list(self.__rpc_connection.listunspent()))
        ...
        # print("COMBINED_DATA", combined_data_hex)

        txouts, change = generate_scriptpubkeys(combined_data_hex, change)
    ...
    def retrieve_data_from_utxo(self, blockchain_transaction_id):
        ...
        # print("COMBINED_DATA", reconstructed_encoded_animecoin_zstd_compressed_data)
