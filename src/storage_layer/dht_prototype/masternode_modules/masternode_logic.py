...
class MasterNodeLogic:
    ...
    async def run_heartbeat_forever(self):
        while True:
            ...
            try:
                response_data = await self.__send_rpc_ping(mn, data)
            except RPCFailed as exc:
    ...
    async def __send_rpc_ping(self, mn, data):

        self.__logger.debug("PING REQUEST to %s" % mn)
        request_msg = ["PING_REQ", data]

        returned_data = await self.__send_rpc_to_mn("PING_RESP", mn, request_msg)
        ...
        self.__logger.debug("PING RESPONSE from %s, msg: %s" % (mn, response_data))

        return response_data

    async def __send_rpc_spotcheck(self, mn, chunkid, start, end):
        self.__logger.debug("SPOTCHECK REQUEST to %s, chunkid: %s" % (mn, int_to_hex(chunkid)))
        ...
        self.__logger.debug("SPOTCHECK RESPONSE from %s, msg: %s" % (mn, response_digest))

        return response_digest

    async def __send_rpc_fetchchunk(self, mn, chunkid):
        self.__logger.debug("FETCHCHUNK REQUEST to %s, chunkid: %s" % (mn, int_to_hex(chunkid)))
