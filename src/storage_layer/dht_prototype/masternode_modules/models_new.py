...
from .model_validators import FieldValidator, StringField, IntegerField, FingerprintField, SHA3512Field, LubyChunkHashField,\
    LubyChunkField, ImageField, ThumbnailField, TXIDField, SignatureField, PubkeyField
from .animecoin_modules.animecoin_dupe_detection import DupeDetector, combine_fingerprint_vectors
...
class ImageData(TicketModelBase):
    ...
    def validate(self):
        ...
        # TODO: image should be a proper image file
...
class RegistrationTicket(TicketModelBase):
    ...
    def validate(self, chainwrapper):
        ...
        try:
            ...
            chainwrapper.retrieve_ticket(self.imagedata_hash)
        except KeyError:
            # artwork does not exist, proceed
            pass
        else:
            raise ValueError("Artwork already exists on chain: %s" % self.imagedata_hash)

        # validate that fingerprints are not dupes
        all_fingerprints = chainwrapper.get_all_fingerprints()
...
class ActivationTicket(TicketModelBase):
    ...
    def validate(self, chainwrapper, image):
        ...
        # TODO remove this mock:                                     vvvvvvvvvvvvvvvv
        assert(regticket.fingerprints == image.generate_fingerprints())
