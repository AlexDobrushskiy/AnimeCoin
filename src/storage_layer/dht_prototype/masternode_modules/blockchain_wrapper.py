...
from .models_new import ActivationTicket, RegistrationTicket

VALID_BLOCKCHAIN_TICKETS = {
    RegistrationTicket,
    ActivationTicket,
}
...
class ChainWrapper:
    ...
    def getbestblockhash(self):
        return self.__blockchain.jsonrpc.getbestblockhash()
    ...
    def store_ticket(self, ticket):
        if type(ticket) == RegistrationTicket:
            identifier = b'regticket'
        elif type(ticket) == ActivationTicket:
            identifier = b'actticket'
        else:
            raise TypeError("Ticket type invalid, valid tickets: %s" % VALID_BLOCKCHAIN_TICKETS)
    ...
    def retrieve_ticket(self, txid):
        ...
        ticket = None
        for identifier in [b'regticket', b'actticket']:
            if raw_ticket_data.startswith(identifier):
                ticket = RegistrationTicket(serialized=raw_ticket_data[len(identifier):])
                break
